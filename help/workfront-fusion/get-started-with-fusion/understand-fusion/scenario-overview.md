...
## Vue d’ensemble des éléments de scénario
...
Un scénario commence par un **déclencheur**. Le déclencheur recherche les données nouvelles et mises à jour et lance le scénario lorsque certaines conditions configurées dans le module s’appliquent. Les déclencheurs peuvent être configurés pour lancer un scénario selon un planning (interrogation) ou chaque fois que des modifications de données se produisent (instantané).
...
+++**Automatiser les processus dans Adobe Workfront**

Workfront Fusion vous permet d’automatiser des workflows simples ou complexes au sein de Workfront, ce qui vous permet de gagner du temps et de vous assurer que le processus est exécuté de manière cohérente.

Dans cet exemple, le scénario se déclenche lorsqu’un champ spécifié est modifié dans une tâche ou un problème dans Workfront. Lorsqu’il est déclenché, le scénario récupère des informations dans le projet concerné et crée une mise à jour personnalisée pour une personne affectée à un rôle spécifique dans le projet.

![Exemple de modèle](assets/fusion-template-example.png)

+++

+++**Connecter Workfront à d’autres applications ou services web**

>[!NOTE]
>
>Si votre entreprise utilise le modèle de licence hérité, elle doit disposer d’une licence Workfront Fusion pour l’automatisation et l’intégration du travail afin de se connecter à d’autres applications.

Workfront Fusion peut se connecter à d’autres applications et services web. Vous pouvez accéder à des données d’autres applications, les importer, les manipuler ou les exporter, pour les intégrer à Workfront ou les unes aux autres.

De nombreuses applications disposent de connecteurs Workfront Fusion dédiés. S’il n’existe pas de connecteur dédié pour l’application à laquelle vous souhaitez accéder, vous pouvez utiliser les modules HTTP ou SOAP de Workfront Fusion pour vous connecter à l’application via son API.

Dans cet exemple, le scénario se déclenche lors de l’ajout d’un utilisateur ou d’une utilisatrice à une feuille de calcul [!DNL Excel]. Le scénario vérifie si l’utilisateur ou l’utilisatrice est dans Workfront. Si ce n’est pas le cas, le scénario crée l’utilisateur ou l’utilisatrice dans Workfront et ajoute son ID Workfront à la feuille de calcul.

![Exemple d’intégration](assets/fusion-integration-example.png)

Pour obtenir la liste des connecteurs dédiés, consultez [Références des applications Fusion et de leurs modules : index des articles](/help/workfront-fusion/references/apps-and-modules/apps-and-modules-toc.md).


>[!IMPORTANT]
>
>Adobe Workfront Fusion peut se connecter à presque tous les services web. Si l’application que vous souhaitez utiliser ne dispose pas d’un connecteur Workfront Fusion dédié, vous pouvez utiliser des connecteurs universels pour vous connecter à l’application ou au service.
>
