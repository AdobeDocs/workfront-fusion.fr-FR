...
* Le premier module d’un scénario ne s’exécute qu’une seule fois et est toujours compté comme une opération, même s’il ne renvoie pas de bundle.
* Le nombre d’exécutions des autres modules dépend du nombre de bundles qu’ils doivent traiter.  Une exécution d’un module pour un bundle constitue une seule opération. Le module agrégateur fait exception à la règle, puisqu’il est compté comme une opération pour chaque ensemble de bundles traités.
...
Votre organisation peut avoir une limite mensuelle d’opérations. Ce montant est basé sur le forfait Workfront acheté par votre entreprise. Le plan ] Workfront [!UICONTROL  offre un nombre illimité d’opérations.
