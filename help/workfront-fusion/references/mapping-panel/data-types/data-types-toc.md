---
content-type: reference
title: 'Références des types de données : index des articles'
description: Cette section contient des ressources de référence relatives aux types de données dans Adobe Workfront Fusion.
author: Becky
feature: Workfront Fusion
recommendations: noDisplay, noCatalog
exl-id: c31c0ad9-6de4-467d-8ccd-9376443e2e7e
TQID: https://experienceleague.adobe.com/O08hruYCfmwW8hSM9R9Kr19sQSiqj0eGcw45YpnVAbA
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
source-git-commit: 219b9dbf3a7e4be1676b21bc3d3752d70d743b13
workflow-type: tm+mt
source-wordcount: 36
ht-degree: 36%

---

# Références des types de données : index des articles

Cette section contient les articles suivants :

* [Types de données d’élément](/help/workfront-fusion/references/mapping-panel/data-types/item-data-types.md)
* [Coercition de type](/help/workfront-fusion/references/mapping-panel/data-types/type-coercion.md)
* [Structures de données](/help/workfront-fusion/references/mapping-panel/data-types/data-structures.md)
